...
- **REQ-JUDGE-2** The gate keys on failure evidence — `failure_type` +
...
| REQ-JUDGE-5 | `BaseJudge.score` try/except | `test_http_failure_falls_back`, `test_malformed_response_falls_back` |
| REQ-JUDGE-6 | `judge_from_env`, `MockJudge` | `TestJudgeFactory`, `TestMockJudge` |
| REQ-JUDGE-7 | `BaseJudge` counters; `worker.report` | `test_successful_scoring` (usage), stats line in E2E |
